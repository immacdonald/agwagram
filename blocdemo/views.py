...
def analysis_results(request, usernames):
    ...
    if results['successful_generation']:
        if(results['query_count'] > 1):
            ...
            context = {"account": format_account_data(results["account_blocs"][0])}
...
def format_account_data(account):
    ...
    if account["first_tweet_date"] != "":
        first_tweet_date = datetime.strptime(
            account["first_tweet_date"], initial_date_format
        ).strftime(output_date_format)
    if account["last_tweet_date"] != "":
        last_tweet_data = datetime.strptime(
            account["last_tweet_date"], initial_date_format
        ).strftime(output_date_format)
    ...
    output_data = {
        ...
        "bloc_semantic_sentiment": process_bloc_string(
            account["bloc_semantic_sentiment"]
        ),
