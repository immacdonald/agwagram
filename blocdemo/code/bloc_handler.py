...
def verify_user_exists(user_list):
    oauth2 = osometweet.OAuth2(
        bearer_token=settings.BEARER_TOKEN, manage_rate_limits=False
    )
...
def analyze_user(usernames):
    ...
    usernames = [
        u for u in usernames if not (u in unique_usernames or unique_usernames.add(u))
    ]
    ...
    if error_count > 0:
        ...
        total_tweets = sum(
            [user_bloc["more_details"]["total_tweets"] for user_bloc in all_bloc_output]
        )
        ...
        if group_bloc_words:
            group_bloc_words = sorted(
                group_bloc_words, key=lambda x: x["term_freq"], reverse=True
            )
        ...
        pairwise_sim_report = sorted(
            pairwise_sim_report, key=lambda x: x["sim"], reverse=True
        )
            ...
            result["account_blocs"].append(
                {
                    ...
                    "first_tweet_date": account_bloc["more_details"][
                        "first_tweet_created_at_local_time"
                    ],
                    "last_tweet_date": account_bloc["more_details"][
                        "last_tweet_created_at_local_time"
                    ],
                    "elapsed_time": account_bloc["elapsed_time"][
                        "gen_tweets_total_seconds"
                    ]
                    + account_bloc["elapsed_time"]["gen_bloc_total_seconds"],
                    # Analysis
                    "bloc_action": account_bloc["bloc"]["action"],
                    "bloc_syntactic": account_bloc["bloc"]["content_syntactic"],
                    "bloc_semantic_entity": account_bloc["bloc"][
                        "content_semantic_entity"
                    ],
                    "bloc_semantic_sentiment": account_bloc["bloc"][
                        "content_semantic_sentiment"
                    ],
