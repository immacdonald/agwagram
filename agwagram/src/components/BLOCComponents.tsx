...
export const DefinitionTooltip: React.FC<BLOCTooltipProps> = ({
  word,
  k,
}: BLOCTooltipProps) => {
...
export const SymbolTooltip: React.FC<BLOCTooltipProps> = ({
  word,
  k,
}: BLOCTooltipProps) => {
  const { symbolToDefinition } = useContext(AnalysisContext);
  return [...word].map((c, index, array) => (
    <Fragment key={k ? `${k}-${index}` : index}>
      {symbolToDefinition(c) != "" ? (
        ...
        <span>{symbolToDefinition(c)}</span>
      )}
