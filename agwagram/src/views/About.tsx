	...
	return (
		<Page title="agwagram">
			<Section>
				...
				<h2>Methodology</h2>
				<p>
					<Link to='https://github.com/anwala/bloc'>BLOC</Link> is a language for representing the online behaviors of social media accounts irrespective of class (human or cyborg or bot) or intent (malicious or benign). BLOC words consist of letters drawn from various alphabets (e.g., actions, pauses, & content alphabets). The language is highly flexible, and can be applied to model a broad spectrum of legitimate and suspicious online behaviors.
				</p>
				...
				<StyledImage image={PCAImage} border caption={<span style={{ opacity: '0.7' }}>2D PCA projections of BLOC TF-IDF vectors of accounts from two datasets that include both humans (blue) and bots (orange) illustrating the discriminative power of BLOC in separating accounts of different classes: (left) <Link to="https://botometer.osome.iu.edu/bot-repository/datasets/cresci-2017/cresci-2017.csv.zip" target="_blank" title="cresci-17">cresci-17</Link> and (right) <Link to="https://botometer.osome.iu.edu/bot-repository/datasets/varol-2017/varol-2017.dat.gz" target="_blank" title="varol-17">varol-17</Link>. The Venn diagrams show the top five pause-delimited BLOC words for the bot and human accounts shown.</span>} />
				<br />

				<h2>Publications</h2>

				<span>A language framework for modeling social media account behavior</span><br />
				<span>Alexander C. Nwala, Alessandro Flammini, and Filippo Menczer</span><br />
				<Link to="https://doi.org/10.1140/epjds/s13688-023-00410-9" target="_blank">EPJ Data Science</Link><br /><br />
